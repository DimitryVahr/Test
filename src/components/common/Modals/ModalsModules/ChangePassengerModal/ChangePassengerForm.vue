<template>
  <form @submit.prevent="">
    <h2 class="forms__title">Изменение данных</h2>
...
export default defineComponent({
  ...
  methods: {
    handleSubmit(): void {
      ...
      if (!this.v$.formModel.$error) {
        ...
        this.v$.$reset()
        this.$services.changePassenger(this.processedFormModal, this.getPassenger.id, this.getAuthConfig).then((res) => {
          this.$store.commit('mutIsLoading', false)
          if (res.status === ERequestStatus.Error) {
            this.$store.commit('mutAlertText', 'Ошибка при изменении данных')
            return
          }
          this.$services.getPassengerInfo(this.getAuthConfig,
            this.getPassenger.id)
            .then((passengerRes) => {
              if (passengerRes.status === ERequestStatus.Success) {
                this.$store.commit('mutPassenger', passengerRes.data)
              }
            })
          ...
          this.$store.commit('mutAlertText', 'Данные успешно изменены')
        })
      }
    }
  },
  ...
  created() {
    this.formModel.firstName = this.getPassenger.firstName
    this.formModel.lastName = this.getPassenger.lastName
    this.formModel.patronymic = this.getPassenger.patronymic
    this.formModel.sex = this.getPassenger.sex
    this.formModel.birthday = this.getPassenger.birthday
    this.formModel.phone = this.getPassenger.phone
    this.stateSelect = this.getStates(true).map((state: ISelectItem) => {
      return {
        ...state, selected: state.value === this.getPassenger.state
      }
    })
    this.formModel.state = this.getPassenger.state
    this.documentTypeSelect = this.getDocumentTypes(true).map((docType: ISelectItem) => {
      return {
        ...docType, selected: docType.value === String(this.getPassenger.documentType)
      }
    })
    this.formModel.documentNumber = this.getPassenger.documentNumber
    this.formModel.documentType = this.getPassenger.documentType
    this.formModel.isLocal = !!this.getPassenger.isLocal
    this.formModel.imagesIds = this.getPassenger.imagesIds
    this.formModel.bonusCard = this.getPassenger.bonusCard || ''
  }
})
</script>
